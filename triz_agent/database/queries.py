...
def get_parameter_by_id(param_id: int) -> Optional[dict]:
    with _get_conn() as conn:
        cursor = conn.cursor()
        cursor.execute(
            "SELECT id, name, name_cn, description FROM parameters WHERE id = ?",
            (param_id,),
        )
        row = cursor.fetchone()
        if row:
...
def get_all_parameters() -> List[dict]:
    with _get_conn() as conn:
        cursor = conn.cursor()
        cursor.execute(
            "SELECT id, name, name_cn, description, embedding_json FROM parameters ORDER BY id"
        )
...
def query_parameters_by_similarity(keyword: str) -> List[dict]:
    ...
    with _get_conn() as conn:
        ...
        rows = cursor.fetchall()
        return [
            {"id": r[0], "name": r[1], "name_cn": r[2], "description": r[3]} for r in rows
        ]
...
def get_separation_principles_by_type(sep_type: str) -> List[int]:
    with _get_conn() as conn:
        cursor = conn.cursor()
        cursor.execute(
            "SELECT principles FROM separation_rules WHERE type = ?", (sep_type,)
        )
        row = cursor.fetchone()
        if row and row[0]:
            return json.loads(row[0])
...
def get_all_separation_types() -> List[dict]:
    with _get_conn() as conn:
        ...
        rows = cursor.fetchall()
        return [
            {"id": r[0], "type": r[1], "condition": r[2], "principles": json.loads(r[3])}
            for r in rows
        ]


def query_cases(
    principle_ids: List[int], function: str = "", limit: int = 10
) -> List[dict]:
    with _get_conn() as conn:
        cursor = conn.cursor()

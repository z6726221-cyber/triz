import pytest
import os
from triz_pipeline.database.init_db import init_database, ensure_data_dir
from triz_pipeline.database.queries import get_parameter_by_id, query_parameters_by_similarity
...
def temp_db(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("triz_pipeline.config.DB_PATH", db_path)
    monkeypatch.setattr("triz_pipeline.database.init_db.DB_PATH", db_path)
    monkeypatch.setattr("triz_pipeline.database.queries.DB_PATH", db_path)
    init_database()
    yield db_path
    if db_path.exists():
        os.remove(db_path)

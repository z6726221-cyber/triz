...
def check_convergence(ctx: WorkflowContext) -> ConvergenceDecision:
    ...
    if not solutions:
        return ConvergenceDecision(action="CLARIFY", reason="无可用方案进行评估")
    ...
    if top_relevance < 3:
        return ConvergenceDecision(
            ...
            feedback=_generate_feedback(
                signals, max_ideality, top_relevance, top_consistency
            ),
        )
    ...
    if not signals:
        return ConvergenceDecision(
            action="TERMINATE", reason="信号已清空，矛盾已充分解决"
        )
    ...
    if iteration >= MAX_ITERATIONS:
        return ConvergenceDecision(
            action="TERMINATE", reason=f"达到最大迭代次数 {MAX_ITERATIONS}"
        )
...
def _generate_feedback(
    signals: list, max_ideality: float, relevance: int, consistency: int
) -> str:
    """生成给下一轮 M5 的反馈信息。"""
    feedback_parts = [

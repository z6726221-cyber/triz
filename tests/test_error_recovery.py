...
def test_orchestrator_catches_api_timeout():
    ...
    with patch.object(orch.skill_runner, 'run', side_effect=TimeoutError("API timeout")):
...
def test_orchestrator_catches_generic_exception():
    ...
    with patch.object(orch.skill_runner, 'run', side_effect=RuntimeError("Boom")):
        ctx = WorkflowContext(question="test")
        ctx.sao_list = [SAO(subject="A", action="B", object="C", function_type="useful")]
        result = orch._execute_node("矛盾求解", 2, 5, ctx, [("m4_solver", "Skill")])
...
def test_orchestrator_clarify_when_empty_sao():
    ...
    orch = Orchestrator()
    with patch.object(orch.skill_runner, 'run', return_value={}):
...
def test_orchestrator_fallback_when_empty_principles():
    ...
    def mock_run(skill_name, ctx, **kwargs):
    ...
    with patch.object(orch.skill_runner, 'run', side_effect=mock_run):
...
def test_orchestrator_fallback_when_empty_drafts():
    ...
    def mock_run(skill_name, ctx, **kwargs):
        if skill_name == "m1_modeling":
            ...
            return {"principles": [1, 15], "improve_param_id": 1, "worsen_param_id": 2}
